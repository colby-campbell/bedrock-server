...
class ServerConfig:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.CONFIG_PATH):
            ...
            print(f"{self.CONFIG_PATH} not found. Sample created. Edit it and rerun.")
            sys.exit()
    ...
    def validate(self):
        ...
        if not self.executable_loc:
            errors.append("Missing executable_location.")
            ...
            errors.append("The executable_location path does not exist.")
        # log_location
        if not self.log_loc:
            errors.append("Missing log_location.")
        elif not isinstance(self.log_loc, str):
            errors.append("log_location must be a string.")
        elif not os.path.exists(self.log_loc):
            errors.append("The log_location path does not exist.")
        # world_location
        if not self.world_loc:
            errors.append("Missing world_location.")
            ...
            errors.append("The world_location path does not exist.")
        # backup_location
        if not self.backup_loc:
            errors.append("Missing backup_location.")
            ...
            errors.append("The backup_location path does not exist.")
        # backup_duration
        if not isinstance(self.backup_dur, int):
            errors.append("backup_duration must be an integer.")
        # restart_time
        if not isinstance(self.restart_time, str):
        ...
        if not isinstance(self.admins, list):
            errors.append("admin_list must be a list containing Discord IDs as integers.")
